...
export const HeaderTop = () => {
  ...
  return (
<Grid container justifyContent="space-between" alignItems="center" sx={{ flexDirection: 'row', backgroundColor:'#922790', padding: '10px' }}>
  {/* Logo */}
  <Grid item xs={3} sm={3} sx={{ display: 'flex', justifyContent: 'center' }}>
    <Link to="/">
      <img src={`${globalConfig.get().apiUrl}/download/${mainInformation.logo_Url}`} alt="Logo" style={{ width: isSmallScreen ? '100px' : '130px', height: '75px', marginTop: '5px' }} />
    </Link>
  </Grid>

  {/* Barre de recherche */}
  <Grid item xs={6} sm={4} sx={{ display: 'flex', justifyContent: 'center', marginBottom: '10px' }}>
    <InputBase placeholder="Recherche..." fullWidth sx={{ borderRadius: 1, border: '1px solid white', fontWeight: 'bold', marginTop: '10px', marginRight:'-23px' }} endAdornment={
      <IconButton>
  ...
  <Grid item xs={3} sm={3} sx={{ display: 'flex', justifyContent: isSmallScreen ? 'center' : 'flex-end', alignItems: 'center', marginBottom: '10px' }}>
    ...
    {isSmallScreen ? (
      <>
        <IconButton color="inherit" sx={{ color: 'white', marginRight: '2px', marginTop: '7px' }}>
          <PersonAddIcon />
        </IconButton>
        <IconButton color="inherit" sx={{ color: 'white', marginTop: '7px' }}>
          <PersonIcon />
        </IconButton>
      </>
    ) : (
      ...
      <>
      <IconButton color="inherit" sx={{ color: 'white', marginRight: '15px', marginTop: '7px' }}>
        <PersonAddIcon />
      </IconButton>
      <Typography variant="body1" sx={{ fontFamily: 'Poppins', color: 'white', marginRight: '15px', marginTop: '7px' }}>
        Nouveau Client
      </Typography>
      <IconButton color="inherit" sx={{ color: 'white', marginLeft: '15px', marginTop: '7px' }}>
        <PersonIcon />
      </IconButton>
      <Typography variant="body1" sx={{ fontFamily: 'Poppins', color: 'white', marginRight: '15px', marginTop: '7px' }}>
        Se connecter
